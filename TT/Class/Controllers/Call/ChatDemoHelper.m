...
- (void)_cancelCall
{
    [self hangupCallWithReason:EMCallEndReasonNoResponse withEmcall:_callSession];
...
- (void)hangupCallWithReason:(EMCallEndReason)aReason withEmcall:(EMCallSession *)calls
{
    _callSession = calls;
...
//- (void)answerCall
- (void)answerCall:(EMCallSession *)calls
{
    _callSession = calls;
    if (_callSession) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            if (error) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (error.code == EMErrorNetworkUnavailable) {
                        ...
                        [self hangupCallWithReason:EMCallEndReasonFailed withEmcall:_callSession];
                    }
                });
            }
        });
